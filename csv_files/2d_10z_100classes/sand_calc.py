...
def rel_error_table_nonormal(folder_name, num_runs, epochs_list):
...
def excel_maker(num_runs, num_classes):
    df_dict_output = dict()
    df_dict_target = dict()
    for run in np.linspace(0, num_runs-1, num_runs, dtype='int'):
...
if __name__ == '__main__':
    ...
    # show_noise()
    epoch_nums = 30
    epochs_list = np.linspace(10, epoch_nums, int(epoch_nums / 10))
    rel_error_table_nonormal(folder_name='2d_10z_100classes', num_runs=8, epochs_list=epochs_list)
    excel_maker(num_runs=8, num_classes=100)
